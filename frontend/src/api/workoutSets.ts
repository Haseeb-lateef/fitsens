...
export function getWorkoutSetsForDate(date: string): Promise<WorkoutSetOut[]> {
  return apiClient<WorkoutSetOut[]>(`/workout-sets?date=${date}`);
}

export function deleteWorkoutSet(setId: number): Promise<void> {
  return apiClient<void>(`/workout-sets/${setId}`, { method: "DELETE" });
}
